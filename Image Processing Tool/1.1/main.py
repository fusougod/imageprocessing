...
class ImageProcessorApp:
    ...
    def create_widgets(self):
        ...
        tk.Label(self.sliders_frame, text="Grayscale").pack(anchor=tk.W)
        ttk.Scale(self.sliders_frame, from_=0, to=1, orient=tk.HORIZONTAL, 
                 variable=self.grayscale_var, command=self.apply_filters).pack(fill=tk.X, padx=5)
        
        # Blur
        self.blur_var = tk.DoubleVar(value=0)
        tk.Label(self.sliders_frame, text="Blur").pack(anchor=tk.W)
        ttk.Scale(self.sliders_frame, from_=0, to=10, orient=tk.HORIZONTAL, 
                 variable=self.blur_var, command=self.apply_filters).pack(fill=tk.X, padx=5)
        
        # Contrast
        self.contrast_var = tk.DoubleVar(value=1)
        tk.Label(self.sliders_frame, text="Contrast").pack(anchor=tk.W)
        ttk.Scale(self.sliders_frame, from_=0.1, to=3, orient=tk.HORIZONTAL, 
                 variable=self.contrast_var, command=self.apply_filters).pack(fill=tk.X, padx=5)
        
        # Brightness
        self.brightness_var = tk.DoubleVar(value=1)
        tk.Label(self.sliders_frame, text="Brightness").pack(anchor=tk.W)
        ttk.Scale(self.sliders_frame, from_=0.1, to=3, orient=tk.HORIZONTAL, 
                 variable=self.brightness_var, command=self.apply_filters).pack(fill=tk.X, padx=5)
        
        # Sharpen
        self.sharpen_var = tk.DoubleVar(value=0)
        tk.Label(self.sliders_frame, text="Sharpen").pack(anchor=tk.W)
        ttk.Scale(self.sliders_frame, from_=0, to=5, orient=tk.HORIZONTAL, 
                 variable=self.sharpen_var, command=self.apply_filters).pack(fill=tk.X, padx=5)
        
        # Rotation
        self.rotation_var = tk.DoubleVar(value=0)
        tk.Label(self.sliders_frame, text="Rotation").pack(anchor=tk.W)
        ttk.Scale(self.sliders_frame, from_=0, to=360, orient=tk.HORIZONTAL, 
                 variable=self.rotation_var, command=self.apply_filters).pack(fill=tk.X, padx=5)
    ...
    def load_image(self):
        ...
        if self.image_path:
            try:
                self.original_image = Image.open(self.image_path).convert("RGB")  # Convert to RGB on load
