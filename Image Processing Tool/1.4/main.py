...
class ImageProcessorApp:
    def __init__(self, root):
        ...
        self.root.geometry("900x700")  # Fixed overall window size
    ...
    def create_widgets(self):
        ...
        # Left frame for image preview (fixed width and height)
        self.left_frame = tk.Frame(self.main_frame, bg="#f0f0f0", width=600, height=700)
        ...
        # Image display
        self.canvas_frame = tk.Frame(self.left_frame, bg="#d3d3d3", bd=2, relief="sunken", width=580, height=580)  # Adjusted for padding
        self.canvas_frame.pack(padx=10, pady=10)
        self.canvas = tk.Canvas(self.canvas_frame, width=580, height=580, bg="#d3d3d3", highlightthickness=2, highlightbackground="#4a90e2")
        self.canvas.pack()

        # Right frame for controls (fixed width and height)
        self.right_frame = tk.Frame(self.main_frame, bg="#f0f0f0", width=280, height=700)  # Adjusted for padding
        self.right_frame.pack(side=tk.RIGHT, fill=tk.Y, padx=5, pady=5)

        # Controls frame with border
        self.controls_frame = tk.Frame(self.right_frame, bg="#f0f0f0", bd=2, relief="groove", width=260, height=650)  # Adjusted for padding
        self.controls_frame.pack(padx=10, pady=10)

        # Button frame
        self.button_frame = tk.Frame(self.controls_frame, bg="#f0f0f0", width=260)
        ...
        self.sliders_frame = tk.Frame(self.controls_frame, bg="#f0f0f0", bd=1, relief="ridge", width=260, height=400)
        ...
        ttk.Scale(self.sliders_frame, from_=0, to=2, orient=tk.HORIZONTAL, 
                 variable=self.saturation_var, command=self.apply_filters, length=200).grid(row=5, column=1, padx=5, pady=1)
        ...
        ttk.Scale(self.sliders_frame, from_=0, to=5, orient=tk.HORIZONTAL, 
                 variable=self.edge_enhance_var, command=self.apply_filters, length=200).grid(row=6, column=1, padx=5, pady=1)
        ...
        ttk.Scale(self.sliders_frame, from_=0, to=360, orient=tk.HORIZONTAL, 
                 variable=self.rotation_var, command=self.apply_filters, length=200).grid(row=7, column=1, padx=5, pady=1)
        ...
        self.crop_frame = tk.Frame(self.controls_frame, bg="#f0f0f0", bd=1, relief="ridge", width=260, height=100)
        self.crop_frame.pack(pady=5)

        # Crop and resize controls
        ttk.Button(self.crop_frame, text="Crop Center (50%)", command=self.crop_image, style="TButton").pack(side=tk.TOP, padx=5, pady=2)
        ttk.Label(self.crop_frame, text="Resolution:", style="TLabel").pack(side=tk.TOP, padx=5, pady=2)
        resolutions = [
        ...
        self.resolution_menu.pack(side=tk.TOP, padx=5, pady=2)
        ttk.Button(self.crop_frame, text="Apply Resize", command=self.resize_image, style="TButton").pack(side=tk.TOP, padx=5, pady=2)
    ...
    def display_image(self, image):
        # Adjust canvas to fit image, but cap at 580x580 to fit fixed frame
        width, height = min(image.width, 580), min(image.height, 580)
        self.canvas.config(width=width, height=height)
        self.photo = ImageTk.PhotoImage(image.resize((width, height), Image.Resampling.LANCZOS))
